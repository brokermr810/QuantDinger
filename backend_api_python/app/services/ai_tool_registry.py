...
TOOLS: tuple[ToolDefinition, ...] = (
    ...
    ToolDefinition(
        id="script_strategy.create_draft",
        category="strategy",
        label_zh="创建停止状态机器人",
        label_en="Create script strategy draft",
        description_zh="创建默认停止状态的交易机器人配置，启动必须由用户手动点击。",
        description_en="Create a script strategy draft; the user confirms backtests and live execution in Strategy IDE.",
        route="/strategy-ide?tab=script",
        requires=("strategy_parameters", "risk_limits"),
        produces=("script_strategy_draft",),
        risk_level="write_draft",
        read_only=False,
        priority=86,
...
MCP_AGENT_TOOLS: tuple[ToolDefinition, ...] = (
    ...
    ToolDefinition(
        ...
        description_zh="提交异步脚本策略回测任务，支持 strict_mode、策略配置和脚本参数。",
        description_en="Submit an async ScriptStrategy backtest with strict_mode, strategy config, and script parameters.",
